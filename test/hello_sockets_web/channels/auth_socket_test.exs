defmodule HelloSocketsWeb.AuthSocketTest do
  ...
  describe "connect/3 failure" do
    test "cannot be connected to with an invalid token" do
  ...
  describe "id/1" do
    test "an identifier is based on the connected ID" do
      params = %{"token" => generate_token(1)}
      assert {:ok, socket} = connect(AuthSocket, params)
